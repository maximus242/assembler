...
(define (apply-relocation code offset type symbol symbol-table code-base-address code-start-offset got-offset)
  ...
  (let* ((symbol-info (safe-ref symbol-table symbol))
    ...
    (if symbol-address
        (let* ((got-entry-offset (- got-offset code-base-address (- symbol-address code-base-address)))
               (relative-address
                (case type
                  ((rip-relative got-pcrel) 
                   (log-message "  Calculating RIP-relative address to GOT entry")
                   (- got-entry-offset (- next-instruction-address code-base-address)))
                  ((absolute) 
                   (log-message "  Using absolute address")
                   symbol-address)
                  ((relative)
                   (log-message "  Calculating relative address")
                   (- symbol-address next-instruction-address))
                  (else (error "Unknown relocation type" type))))
